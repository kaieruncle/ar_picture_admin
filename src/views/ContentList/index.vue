<template>
  <div class="content_list_container">
    ...
    <!-- <div class="table_btn_box">
      <a-button type="primary" @click="wakeUpHandleInfoModal">新增内容</a-button>
    ...
    <a-table :columns="columns" :data-source="list" :loading="loading" :pagination="pagination" :scroll="{ x: 1200 }"
      ...
      <template #bodyCell="{ column, record }">
        ...
        <a-tag v-if="column.dataIndex === 'is_deleted'" :color="record.is_deleted ? 'red' : 'green'">{{
          record.is_deleted ? '已删除' : '正常' }}</a-tag>
        <a-image v-if="column.dataIndex === 'picture_url'" :width="60" :height="60" :src="record.picture_url"></a-image>
        <a-image v-if="column.dataIndex === 'ew_code_url' && !!record.ew_code_url" :width="60"
          :src="record.ew_code_url"></a-image>
        <span v-if="column.dataIndex === 'ew_code_url' && !record.ew_code_url">-</span>

        <div v-if="column.dataIndex === 'operation'" class="table_operation">
          <span @click="wakeUpRegenQrcodeConfirm(record)" v-if="record.status === 'release'">刷新码</span>
...
import { getpicturelist, deletepicture, postpictureregenqrcode } from './api';
...
const getList = async () => {
  ...
  const { list: newList, count } = await getpicturelist(payload)
  list.value = newList || []
...
const wakeUpRegenQrcodeConfirm =  record => {
  ...
  Modal.confirm({
    ...
    onOk: async () => {
      await postpictureregenqrcode({id});
